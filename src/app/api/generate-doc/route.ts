...
### 4. 스토리 포인트
복잡도 기반 추정 (1, 2, 3, 5, 8, 13)

### 5. 우선순위
- **P0 (Must-have)**: MVP 필수
- **P1 (Should-have)**: 초기 릴리스 포함
- **P2 (Nice-to-have)**: 이후 릴리스
...
|--------|------|------|-----|------|--------|
| id | UUID | NOT NULL | PK | 사용자 ID | uuid_generate_v4() |
| email | VARCHAR(255) | NOT NULL | UQ | 이메일 | |
| password_hash | VARCHAR(255) | NOT NULL | | 비밀번호 해시 | |
| created_at | TIMESTAMP | NOT NULL | | 생성일시 | CURRENT_TIMESTAMP |
| updated_at | TIMESTAMP | | | 수정일시 | |

**인덱스:**
- \`idx_users_email\`: (email) - 로그인 최적화
...
### 4. 관계 정의
| 테이블 A | 관계 | 테이블 B | 카디널리티 | 설명 |
|----------|------|----------|-----------|------|
| users | 1:N | orders | 한 사용자는 여러 주문을 가짐 | |
