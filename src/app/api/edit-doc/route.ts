...
export async function POST(request: NextRequest) {
  ...
  const system = [
    ...
    '4. 웹 검색 결과를 쓸 땐 추측이 아닌 검색으로 확인된 사실을 우선한다. 가공의 회사명/가명("로컬 B사" 등)은 실제 이름으로 대체한다.',
    '5. content 안의 줄바꿈/따옴표는 JSON 규칙에 맞게 이스케이프한다.',
  ].join('\n');

  // 리서치 필요 휴리스틱: 외부 사실을 요구하는 신호가 있으면 web_search 부착.
  const researchSignals = /경쟁사|시장|점유율|통계|트렌드|동향|벤치마|레퍼런스|사례|실제|최신|규모|순위|업계|표준|가격|요금|평균|리서치|조사|찾아|검색/;
  const wantsResearch = researchSignals.test(instruction);
  ...
  try {
    const result = await llmComplete({
      ...
      timeoutMs: 280_000,
      maxRetries: 1,
      enableWebSearch: wantsResearch, // 리서치 신호 있으면 GLM web_search 부착
    });
