...
1. **일괄 갱신 구현 방식** — PrdViewer 로컬 순차루프(빠름·재개 없음) vs `store.regenerateDocs` 인프라 통합(resumable).
   → **권장: store 통합(1~4번)**. 19분급이라 모바일 백그라운드/새로고침 끊김 일상적 → resumable 필수. 로컬 루프는 끊기면 0건=P0 버그 일괄판 재현. 추가 비용은 mode 필드+`levelsFor`+상태전이 훅뿐, 새 잡/락 없어 좀비 위험 0.

2. **일괄갱신 진행 중 GenerationGuard 전면 딤** vs 작은 진행바.
   → **권장: 작은 진행바만**. 일괄은 보통 2~5개로 짧고 다른 문서 보며 대기 원할 수 있음. regen 모드일 땐 GenerationGuard 오버레이 스킵 분기 필요(generationProgress 공유 시 자동 전면 딤).

3. **'현재 브라우저 기준' 고지(10번) 채택 여부** — 정확하나 다기기 불일치 불안 신규 유발 가능.
   → **권장: major 영향배너 1회 한정으로 채택**. 본문은 동기화되는데 배지만 다른 비대칭이 '데이터 유실'로 오인되기 가장 쉬운 지점.

4. **frozen 문서 상위 변경 시 동작** — 현재 '업데이트 필요' 미표시(9번 노트 고지) vs '상위변경됨, 단 잠금됨' 별도 표시.
   → **권장: 인라인 노트로 충분(9번)**. 별도 상태는 docStatuses 단일슬롯에 새 상태 끼우거나 frozen 우선반환 깸 → 복잡·회귀. 필요하면 사용자가 unfreeze→재생성.

5. **일괄 갱신이 frozen 부모로 끊긴 사슬을 만났을 때** — frozen 건너뛰고 후손 갱신 vs 사슬 끊고 멈춤.
   → **권장: frozen은 컨텍스트 시드로만 쓰고 갱신 대상에서 제외, 후손은 정상 갱신**. frozen=사용자가 '이 내용 유지' 선언 → 그 내용으로 후손 갱신은 사용자 의도 존중. 7번 가드는 outdated 부모만 잡으므로 충돌 없음.
