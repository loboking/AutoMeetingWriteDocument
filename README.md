...
5. **기획 문서 생성**
   ...
   > ℹ️ **전체 생성 동작 안내**: 문서 생성은 페이지를 떠나거나 새로고침해도 다시 방문하면 남은 문서부터 이어서 진행됩니다. 진행 중에는 우측 하단 진행바에서 상태 확인 및 "종료"가 가능합니다. 단, **브라우저의 모든 탭을 닫으면 생성이 멈추며, 다른 기기/브라우저에서는 이어지지 않습니다.** (브라우저 로컬에 진행 상황을 저장하는 방식)
